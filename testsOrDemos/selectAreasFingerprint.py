...
#arbitrarily set save path
saveDir='/media/dan/storage/data/exploreTractography/24_C_investigation'
...
for iInflate in range(3):
    ...
    #also include a merged endpoint mask in order to force ending in this specified motif
    leftEndpointROI=wmaPyTools.roiTools.multiROIrequestToMask(modifiedCharmAtlas,leftSourceLabel+leftTargetLabels,inflateIter=0)
    rightEndpointROI=wmaPyTools.roiTools.multiROIrequestToMask(modifiedCharmAtlas,rightSourceLabel+rightTargetLabels,inflateIter=0)
    ...
    leftStreamsSubset=wmaPyTools.segmentationTools.segmentTractMultiROI(streamlines, [leftSourceROI, leftTargetROI,leftEndpointROI,bgElim], [True,True,True,False], ['either_end','either_end','both_end','either_end'])
    rightStreamsSubset=wmaPyTools.segmentationTools.segmentTractMultiROI(streamlines, [rightSourceROI, rightTargetROI,rightEndpointROI,bgElim], [True,True,True,False], ['either_end','either_end','both_end','either_end'])
    ...
    wmaPyTools.visTools.radialTractEndpointFingerprintPlot(streamlines[leftStreamsSubset],modifiedCharmAtlas,charmLUT,tractName=leftName,saveDir=saveDir)
    wmaPyTools.visTools.radialTractEndpointFingerprintPlot(streamlines[rightStreamsSubset],modifiedCharmAtlas,charmLUT,tractName=rightName,saveDir=saveDir)
    
    wmaPyTools.visTools.dipyPlotTract(streamlines[leftStreamsSubset],refAnatT1=None, tractName=os.path.join(saveDir,leftName))
    wmaPyTools.visTools.dipyPlotTract(streamlines[rightStreamsSubset],refAnatT1=None, tractName=os.path.join(saveDir,rightName))
    
    wmaPyTools.visTools.densityGifsOfTract(streamlines[leftStreamsSubset],T1,saveDir=saveDir,tractName=leftName)
    wmaPyTools.visTools.densityGifsOfTract(streamlines[rightStreamsSubset],T1,saveDir=saveDir,tractName=rightName)
    ...
    wmaPyTools.visTools.multiTileDensity(streamlines[leftStreamsSubset],T1,saveDir,leftName,noEmpties=True)
    wmaPyTools.visTools.multiTileDensity(streamlines[rightStreamsSubset],T1,saveDir,rightName,noEmpties=True)
    
    wmaPyTools.streamlineTools.stubbornSaveTractogram(streamlines[leftStreamsSubset],os.path.join(saveDir,leftName+'.trk'))
    wmaPyTools.streamlineTools.stubbornSaveTractogram(streamlines[rightStreamsSubset],os.path.join(saveDir,rightName+'.trk'))
